...
Usage:
  access.py [--root DIR] [--sshd-config FILE] [--passwd FILE] [--sudoers FILE]
            ...
            [--weak-algorithm NAME ...] [--max-auth-tries N] [--previous FILE] [--json]
  access.py --explain RANK|ID             the chain behind one finding
...
def text_report(s, rep, target, standards, fixes=None, previous=None):
    ...
    if any(i["id"] == "access.single-path" for i in findings):
        out += ["", paint("next", "head") + "  close access.single-path before anything else, because "
                    "every other fix here needs a second way in, then `--explain 1` for the chain "
                    "behind a line"]
    elif findings:
        out += ["", paint("next", "head") + "  take the findings in ladder order, `--explain RANK` "
                    "prints what a line means, where it comes from, the fix and the way back"]
    else:
...
def main(argv=None):
    ...
    if a.json:
        ...
        print(text_report(s, rep, target, standards, load_fixes(a.fixes or None),
                          previous_measures(a.previous) if a.previous else None))
    return 0
