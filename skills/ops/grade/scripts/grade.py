...
Usage:
  grade.py [--root ~/dx] [HOST ...] [--today YYYY-MM-DD] [--only ID ...] [--json]
  grade.py [--root ~/dx] [HOST ...] --write        write the verdicts into the log
...
measures the host itself, so a stale audit is reported instead of guessed at.
...
THEME = "ops"               # this theme's log folder in the host folder (decisions/0015)
...
# A count compares exactly. The other families move under a host that is being used, so a
...
def grade_row(row, found):
    ...
    if not tool:
        if out["check"].split(".", 1)[0] in PLANNED:
            out["note"] = (f"{out['check']} waits for its tool: leave the row at `todo` with an "
                           "empty verify date until that release measures it")
        else:
...
def report(host, graded, today):
    ...
    out = [f"grade  {host}  {today.isoformat()}"]
    if not graded:
    ...
    for g in graded:
        out += ["", f"{g['id']}  {g['check']} on {short(g['target']) if g['target'] else 'this host'}"]
        if g["verdict"]:
...
def main(argv=None):
    ...
    for h in a.hosts:         # a folder name, never a path: keep every write inside --root
    ...
    if not folder.is_dir():
        ...
        return 2
    names = a.hosts or sorted(p.name for p in folder.iterdir() if p.is_dir() and (p / "config.md").exists())
    if not names:
        print(f"no host folder under {folder}: run `jorekai-ops:setup` first")
        return 2
    results, texts = {}, []
    for m in names:
    ...
    if a.json:
        print(json.dumps({"tool": "grade", "date": today.isoformat(), "hosts": results},
