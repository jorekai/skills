#!/usr/bin/env python3
"""The month on this host: what changed, what was done, and what is still open.

Usage:
  report.py [--root ~/dx] [HOST ...] --month YYYY-MM [--today YYYY-MM-DD] [--json]
  report.py [--root ~/dx] [HOST ...] --month YYYY-MM --write
...
reporting a host nobody looked at.

`--write` renders machines/<host>/reports/ops/<month>.md from templates/report.md. Every number
...
THEME = "ops"               # this theme's log and report folder in the host folder
...
def pick(entries, first, last):
    ...
    The opening one is the newest audit before the month, because that is the host the month
...
def main(argv=None):
    ...
    for m in a.hosts:         # a folder name, never a path: keep every write inside --root
        if "/" in m or m.startswith("."):
            sys.exit(f"not a host folder name: {m!r}")
    folder = root / "machines"
    if not folder.is_dir():
        print(f"no workspace at {root}: run `jorekai-ops:setup` first")
        return 2
    names = a.hosts or sorted(p.name for p in folder.iterdir()
    ...
    if a.json:
        print(json.dumps({"tool": "report", "month": month, "hosts": results},
