...
def text_report(count, rep, target, standards, fixes=None, previous=None):
    ...
    if findings:
        out += ["", paint("next", "head") + "  rotate every value before anything else is edited",
                paint("      gate: cred.* in the fixes table of jorekai-security:security", "dim")]
    else:
...
def main(argv=None):
    ...
    if a.json:
        ...
        print(text_report(len(files), rep, target, standards, load_fixes(a.fixes or None),
                          previous_measures(a.previous) if a.previous else None))
    return 0
