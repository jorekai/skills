---
name: pipeline
description: "What the forge runs with this repository's own rights, in one pass via scripts/pipeline.py: a privileged trigger that checks out code from a fork, a shell step that puts a value from outside straight into the command line, a workflow that names no rights for its token or grants a broad one, a third-party action bound to a tag rather than to a commit, and a self-hosted runner a trigger from outside can reach. Use when asked whether the build can be taken over, before adding a workflow that touches a pull request from a fork, after a dependency of the build changed, or as the pipeline half of a security sweep."
---
...
2. **Rank the findings, do not list them.** Order by the ladder in the router, not by how many lines a check touched. Look each id up in [../security/references/fixes.md](../security/references/fixes.md) for the fix, the class, and the gate. The pass already ranks by that ladder, because the `Rung` column of that file is the same table, and `--explain RANK` prints the chain behind one line: what it means, where it comes from, the fix, the way back, and what closes it. The answer is one table, `check id | cost | targets | fix | class`, at most five rows, one row per check id; the rest of the shape is in the router's `## Writing the answer`.
...
- A workflow with rights on every job and none at the top is settled. A workflow with rights on some jobs is not: the job without them is the one that gets the default. `write-all`, and a bare `write`, settle nothing either. Both name every scope at once, which the check treats as unsettled.
...
- A self-hosted runner is machine trust, not token rights. A trigger from outside lets any user who can open it run a job there, whatever the token can do, and that job can leave code behind for the next run. `push` and a manual run are not outside triggers, because both already need write access.
