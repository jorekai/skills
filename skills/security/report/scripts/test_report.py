...
Run: python3 skills/security/report/scripts/test_report.py
...
def log(base, actions=(), outcomes=(), name="2026-W37.md"):
    (base / "log" / "security" / name).write_text(
    ...
    def test_an_action_belongs_to_the_month_it_was_applied_in(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            log(base, actions=[action("2026-W37-01", "backup.stale"),
                               action("2026-W37-02", "vuln.ssrf", applied="2026-08-30")])
            out = run(root)
    ...
    def test_a_verdict_written_by_the_grade_skill_is_counted(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            log(base, actions=[action("2026-W37-01", "backup.stale", status="won"),
                               action("2026-W37-02", "vuln.ssrf", status="returned")])
    ...
    def test_a_row_nobody_has_carried_out_does_not_read_as_measuring(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            log(base, actions=[action("2026-W37-01", "backup.stale", status="todo"),
                               action("2026-W37-02", "vuln.ssrf", status="applied")])
...
class UnreadableRowTest(unittest.TestCase):
    ...
    def test_a_malformed_row_is_counted_and_named_by_file_and_line(self):
        with tempfile.TemporaryDirectory() as d:
            root, base = workspace(d)
            log(base, actions=[action("2026-W37-01", "cred.tracked"),
    ...
    def test_a_verify_after_that_is_not_a_date_is_counted(self):
        with tempfile.TemporaryDirectory() as d:
            root, base = workspace(d)
            log(base, actions=[action("2026-W37-01", "cred.tracked", verify="not-a-date")])
            out = run(root)
            self.assertEqual(len(out["unreadable"]), 1)
...
class MovementTest(unittest.TestCase):
    def test_a_cost_that_fell_between_two_audits_is_the_movement(self):
        with tempfile.TemporaryDirectory() as d:
            root, base = workspace(d)
            audit(base, "2026-08-30", "deps", [measured("backup.stale", 40)])
            audit(base, "2026-09-20", "deps", [measured("backup.stale", 12)])
    ...
    def test_two_units_of_the_same_family_still_compare(self):
        with tempfile.TemporaryDirectory() as d:
            root, base = workspace(d)
            audit(base, "2026-08-30", "deps", [measured("backup.stale", 2, "GB")])
            audit(base, "2026-09-20", "deps", [measured("backup.stale", 512, "MB")])
            moved = run(root)["movement"]
    ...
    def test_two_units_of_different_families_are_left_out(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root, base = workspace(d)
            audit(base, "2026-08-30", "deps", [measured("backup.stale", 40, "GB")])
            audit(base, "2026-09-20", "deps", [measured("backup.stale", 3, "count")])
    ...
    def test_a_month_with_no_audit_inside_it_says_so(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(any("no deps audit inside the month" in n for n in out["notes"]))

    def test_one_audit_and_nothing_before_it_opens_the_next_report(self):
        with tempfile.TemporaryDirectory() as d:
            root, base = workspace(d)
            audit(base, "2026-09-20", "deps", [measured("backup.stale", 40)])
            out = run(root)
            self.assertEqual(out["movement"], [])
