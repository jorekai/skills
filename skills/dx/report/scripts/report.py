#!/usr/bin/env python3
"""The month on this machine: what changed, what was done, and what is still open.

Usage:
  report.py [--root ~/dx] [MACHINE ...] --month YYYY-MM [--today YYYY-MM-DD] [--json]
  report.py [--root ~/dx] [MACHINE ...] --month YYYY-MM --write
...
reporting a machine nobody looked at.

`--write` renders machines/<machine>/reports/dx/<month>.md from templates/report.md. Every number
...
THEME = "dx"                # this theme's log and report folder in the machine folder
...
# of a machine, so a report that took every file in it would tell a machine's story with a host's
# numbers. Duplicated from the grade skill's table on purpose: each skill stays standalone.
TOOLS = ("repos", "machine", "github", "agent-config", "friction")
...
def pick(entries, first, last):
    ...
    The opening one is the newest audit before the month, because that is the machine the month
...
def render(machine, month, data):
    ...
    actions = table(["id", "Check", "Target", "What happened", "Verdict"],
                    [[r.get("id", ""), r.get("check", ""), r.get("target", ""),
                      r.get("action", ""), data["verdict_of"].get(r.get("id", ""), "")]
...
def main(argv=None):
    ...
    folder = root / "machines"
    if not folder.is_dir():
        print(f"no workspace at {root}: run `jorekai-dx:setup` first")
        return 2
    names = a.machines or sorted(p.name for p in folder.iterdir()
                                 if p.is_dir() and (p / "config.md").exists())
    if not names:
        print(f"no machine folder under {folder}: run `jorekai-dx:setup` first")
        return 2
    results, texts = {}, []
    for m in names:
    ...
    if a.json:
        print(json.dumps({"tool": "report", "month": month, "machines": results},
