...
Writes `machines/<machine>/reports/dx/YYYY-MM.md`. `jorekai-dx:and-now` answers "what next" in the middle of the loop; this answers "what did the month do to this machine". Every number comes from files that already exist: the audits in the workspace and the log weeks of the month.

Needs the workspace from `jorekai-dx:setup`, one audit inside the month, and one before it. Missing: the report says which half is missing rather than reporting a machine nobody measured.
...
1. **Run the pass for the month.**
   ...
   ```bash
   python3 scripts/report.py --root <workspace> <machine> --month YYYY-MM
   ```
...
3. **Write the file.**
   ...
   ```bash
   python3 scripts/report.py --root <workspace> <machine> --month YYYY-MM --write
   ```

   Done when `reports/dx/YYYY-MM.md` holds no placeholder and every action row carries a log id.

4. **Turn the three next steps into rows.** The report names the three open rows at the top of the ladder. Each one is already a row, or it becomes one now with `scaffold.py --append-row` in `jorekai-dx:setup`, so the next month grades what this one proposed.
