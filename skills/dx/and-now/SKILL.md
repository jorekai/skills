...
- `unreadable` counts a log row the script cannot parse at all: a malformed table row, or an applied row whose `Verify after` cell is not a `YYYY-MM-DD` date. Neither vanishes silently (decisions/0030); the first one names its file and line so it can be repaired by hand.
