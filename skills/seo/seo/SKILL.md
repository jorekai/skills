...
3. `jorekai-seo:gsc-review` prints seven buckets: `striking-q`, `striking-p`, `ctr-gap-q`, `ctr-gap-p`, `decay`, `cannibal`, `not-indexed`. Each bucket head is a bold label with its parameters (`striking-q  8 queries  pos 8 to 20  min 50 impressions`), then a dimmed column header line, then rows of fixed width: text left, numbers right. A bucket with nothing to show prints one dimmed line naming why.
