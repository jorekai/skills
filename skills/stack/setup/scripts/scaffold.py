#!/usr/bin/env python3
"""Scaffold and inspect the stack workspace.

Usage:
  scaffold.py [--root ~/stack] [SLUG ...]    create folders and files; never overwrites
  scaffold.py [--root ~/stack] --check       list missing files, directories and template sections
  scaffold.py [--root ~/stack] --flags       the arguments each measuring script takes
  scaffold.py [--root ~/stack] --log         this week's log path, the next action id, its trailer
  scaffold.py [--root ~/stack] --due         actions whose verify-after date has passed
  scaffold.py [--root ~/stack] --snapshot    copy the repository's stack.yaml into its folder
  scaffold.py [--root ~/stack] --append-row --check-id ID --target T --action A --class C
                                --then "N unit" [--status applied]

The workspace is this theme's own private repository, one folder per repository under repos/
(decisions/0035). The declaration a repository carries, stack.yaml, lives in that repository;
the workspace keeps a snapshot of it and every finding about it.

SLUG defaults to the name of the current directory. Stdlib only.
Exit code 1 only when --check finds something missing or --snapshot has nothing to copy.
"""
import os
import argparse
...
def check(root):
    ...
    for s in found:
        base = root / "repos" / s
        if value(base / "config.md", "role") != ROLE:
            continue                    # a folder that does not say `role: stack` is not measured
...
def value(path, key):
    ...
    A template line may carry a default and an explanation, `profile: standard (what that means)`.
...
def values(path, key, sep=","):
    ...
    `accepted` uses a semicolon, because each entry carries spaces of its own.
    """
    v = value(path, key)
    return [p.strip() for p in v.split(sep) if p.strip()] if v else []


def snapshot_path(root, slug):
    """Where the workspace keeps its copy of the repository's declaration."""
...
def flags(root, slug, today):
    ...
    print(f"state: {value(cfg, 'state') or '(not recorded: new or existing)'}")
    print(f"profile: {value(std, 'profile') or '(no profile, choose starts from standard)'}")

    accepted = [f"--accept {quote(e)}" for e in values(cfg, "accepted", sep=";")]
    common = ([f"--root {quote(path)}", f"--today {today.isoformat()}"] if path else []) + accepted
    snap = snapshot_path(root, slug)
...
def snapshot(root, slug):
    """Copy the repository's declaration into its workspace folder, so the passes read one file."""
    cfg = root / "repos" / slug / "config.md"
    path = value(cfg, "path")
    if not path:
        print(f"path is blank in {cfg}, so there is no declaration to copy")
        return 1
    source = Path(path).expanduser() / DECLARATION
    if not source.is_file():
        print(f"no {DECLARATION} at {source}: run jorekai-stack:choose first")
        return 1
    target = snapshot_path(root, slug)
...
def log(root, slug, today):
    ...
    print(f"commit trailer: Stack-Log: {nid}")
...
def main():
    ...
    ap.add_argument("--snapshot", action="store_true",
                    help="copy the repository's stack.yaml into its workspace folder")
    ...
    ap.add_argument("--target", default="", help="the file, package, or port the action is about")
    ...
    if a.flags:
        ...
        return
    if a.snapshot:
        sys.exit(max(snapshot(root, s) for s in names))
    if a.append_row:
        for s in names:
