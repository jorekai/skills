// Talks to the third target's object store over the S3 protocol at its global endpoint,
// region `auto`, virtual-host addressing. Reads STORAGE_URL as `s3://<bucket>@<endpoint>` and
// STORAGE_KEY as `<access key id>:<secret>`.
import {
  DeleteObjectCommand,
  GetObjectCommand,
  PutObjectCommand,
...
function target(): { bucket: string; endpoint: string; id: string; secret: string } {
  if (!env.STORAGE_URL || !env.STORAGE_KEY) {
    throw new Error("STORAGE_URL or STORAGE_KEY is empty: the tigris adapter needs both");
  }
  const match = /^s3:\/\/([^@]+)@(.+)$/.exec(env.STORAGE_URL);
  ...
  return { bucket: match[1], endpoint: `https://${match[2]}`, id, secret };
}
