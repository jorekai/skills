// Talks to the second target's object store over the S3 protocol at the account endpoint,
...
function target(): { bucket: string; endpoint: string; id: string; secret: string } {
  if (!env.STORAGE_URL || !env.STORAGE_KEY) {
    throw new Error("STORAGE_URL or STORAGE_KEY is empty: the r2 adapter needs both");
  }
