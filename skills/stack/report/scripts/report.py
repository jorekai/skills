...
Usage:
  report.py [--root ~/stack] [REPO ...] --month YYYY-MM [--today YYYY-MM-DD] [--json]
  report.py [--root ~/stack] [REPO ...] --month YYYY-MM --write
...
`--write` renders repos/<slug>/reports/stack/<month>.md from templates/report.md. Every number
...
THEME = "stack"             # this theme's log and report folder in the repository folder
...
# theme's audit would otherwise tell this repository's story with that theme's numbers. Duplicated from the grade skill's table on purpose: each skill stays standalone.
TOOLS = ("guards", "drift")
...
RUNG = {"decl.absent": 1, "decl.unmatched": 1,
