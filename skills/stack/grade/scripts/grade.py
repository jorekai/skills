#!/usr/bin/env python3
"""Grade stack log rows whose verify date has passed: recompute the measure, propose the verdict.

Usage:
  grade.py [--root ~/stack] [REPO ...] [--today YYYY-MM-DD] [--only ID ...] [--json]
  grade.py [--root ~/stack] [REPO ...] --write       write the verdicts into the log
...
THEME = "stack"             # this theme's log folder in the repository folder
...
TOOL_OF = {"decl": "drift", "boundary": "drift", "adapter": "drift", "lock": "drift",
           "escape": "guards", "guard": "guards", "dead": "guards"}
    ...
    if target and by:
        for key, v in by.items():
            if same_target(key, target):
                if v is None:
                    return None, "", "this target was not measured; run the pass after a full gate"
                return float(v), m["unit"], ""
    if m.get("value") is None:
        return None, "", "the measure was not taken, run the pass after a full gate"
    if target and by:
...
def main(argv=None):
    ...
    if not names:
        print(f"no repository folder under {folder}: run `jorekai-stack:setup` first")
        return 2
    results, texts = {}, []
    for m in names:
        if not (folder / m).is_dir():
            print(f"no folder {folder / m}: run `jorekai-stack:setup {m}` first")
            return 2
